...
Agent identity is the trust anchor under the [registry](/concepts/registry). The registry authorizes reads with the `REGISTRY_KEY` battery; human publishing uses the OAuth credential stored by `buttons login`, while machine/CI publishing uses `$BUTTONS_BAT_REGISTRY_WRITE_KEY`. The agent's Ed25519 signature proves *which device* is acting — these boundaries are complementary. Enroll once with `buttons agent setup`, then use `buttons add`, `buttons install`, and `buttons publish` against the configured registry.
