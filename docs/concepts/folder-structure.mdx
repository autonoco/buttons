...
The script file for [code buttons](/buttons/code) and [file buttons](/buttons/file). The extension matches the runtime: `main.sh` for shell, `main.py` for Python, `main.js` for Node. HTTP and agent-only buttons do not have a `main.*` file.

## AGENT.md

Present on [agent buttons](/buttons/agent). Contains the raw instruction text stored at create time via `--agent`. When pressed, the contents are returned in the `agent_prompt` field of the JSON output.
