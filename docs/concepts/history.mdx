...
There is no separate dependency manifest or lockfile. An installed button is pinned **in place**: each installed `button.json` carries `source`, `source_name`, `version`, and `content_hash` (the SHA256 of its content at install time). Committing the button commits its pin, so a teammate gets the same version and content hash on checkout. `history.json` stays local — it is each machine's own audit trail.
