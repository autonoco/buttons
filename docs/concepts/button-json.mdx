...
| `source_name` | Exact name fetched from `source`, such as `@your-desk/hello`. Used by `buttons status` and `buttons update` to find newer versions. |
| `content_hash` | Hash recorded at install time for drift/update checks. Set by install tooling, never `create`. |

Example installed registry stamp:
...
If `@your-desk/hello` is later published as `1.1.0`, `buttons status` can report the newer version and `buttons update` can fetch it from the stamped `source`.
