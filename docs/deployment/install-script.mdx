...
```bash
curl -fsSL https://raw.githubusercontent.com/autonoco/buttons/main/install.sh \
  | BUTTONS_VERSION=v0.128.0 BUTTONS_INSTALL_DIR=$HOME/.local/bin sh
```

| Variable | Default | Description |
|---|---|---|
| `BUTTONS_VERSION` | `latest` | Pin to a specific release tag (e.g. `v0.128.0`). `latest` resolves the newest release via the GitHub API. |
