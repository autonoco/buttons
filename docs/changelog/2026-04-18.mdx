...
The board dashboard now feels more responsive when you press a button.

### Updates

- **Press animation** — pressing a button from the board now plays a quick 4-frame pulse that mimics a physical button press. The animation runs on both keyboard and mouse input, so every press feels tactile regardless of how you trigger it.

- **Live elapsed counter** — while a button is running, the board shows a live clock next to it. Short presses display tenths of a second (e.g. "3.2s"), longer ones switch to minutes and seconds ("2:07"), and anything over an hour shows a full timestamp ("1:04:22"). The counter clears as soon as the press finishes. See the [CLI reference](/cli/buttons_board).
</Update>
