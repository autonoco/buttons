...
- **`buttons logs` command** — run `buttons logs <name>` to open a full-screen viewer that streams every line of stdout and stderr as the button's script writes it. The viewer stays open after the press finishes so you can scroll through the output. Press `Esc` or `q` to dismiss, or `Ctrl+C` to cancel the in-flight press. See the [CLI reference](/cli/buttons_board).

- **Follow mode** — output automatically scrolls to the latest line by default. Press `f` to toggle follow on or off, `g` to jump to the top, and `G` to jump back to the tail. Useful for long-running scripts where you want to check earlier output without losing your place.

- **Severity coloring** — stdout and stderr lines are visually distinguished with different colors and labels, so you can spot errors at a glance without reading every line.
</Update>
