...
<Update label="2026-04-18" description="Board themes via BUTTONS_THEME">
## Board themes via `BUTTONS_THEME`

You can now switch the board's color palette to match your terminal aesthetic.

### New features

- **Four built-in themes** — set the `BUTTONS_THEME` environment variable to change the board's look. Choose from `default` (adapts to your terminal's light or dark background), `paper` (a clean light palette), `phosphor` (green-on-black CRT style), and `amber` (warm VT220 terminal). Each theme adjusts every color role — primary, muted, active indicator, warnings — so the board stays readable in any palette. See the [CLI reference](/cli/buttons_board).
</Update>
