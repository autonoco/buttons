...
- **`buttons logs` command** — run `buttons logs <name>` to open a full-screen viewer that streams every line of stdout and stderr as the child process writes it. The viewer stays open after the press finishes so you can scroll through the output. Press Esc or `q` to dismiss, or Ctrl+C to cancel the in-flight press. See the [CLI reference](/cli/buttons_board).
