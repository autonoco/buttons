...
## Buttons is live
  ...
  **`buttons update` command.** You can now update Buttons to the latest version directly from the CLI. It downloads the correct binary for your OS, verifies the SHA256 checksum, and atomically replaces the running binary. Current releases use `buttons status` for read-only update checks. See the [CLI reference](/cli/buttons_update) for details.
