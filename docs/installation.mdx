...
This checks GitHub Releases for the CLI and the stamped source for installed registry buttons, verifies hashes, and applies available updates. Use `buttons status` for a read-only check.
