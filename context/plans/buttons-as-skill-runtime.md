...
- **Your two hard constraints are satisfied natively:** `buttons install` **copies** button
...
| **`buttons store`** stub → flatten to top-level `install`/`search`/`publish` | ❌ **not implemented** — `// TODO Phase 5.1 (#274)` → "not yet implemented" |
| Button **version / tags / provenance** | ❌ none (only an on‑disk `schema_version`) |
...
- **`batteries`** (`BUTTONS_BAT_<KEY>` injected per press, redacted in `list`) — the secret mechanism a
  button should declare requirements against (ship KEY *names*, never values).
...
1. **Schema (the load‑bearing unit).** Two tiers:
   - **Shipped (#186):** `Version`, `Tags` (discovery + **collections** — `install tag:<x>`), `Requires`
     (peer buttons = deps), `RequiresBatteries` (**env-var key names**, `BUTTONS_BAT_<KEY>`), `Source`,
     `ContentHash` on `button.json`. All `omitempty`. **No `pack.json`** — collections are tags, groups are
     drawers (both already exist).
   - **Architecture-aligned target (add as the IAM layer lands):** per button — `Mutates bool` + `RiskLevel`
     (read vs. mutation; gates the call-time confirm), **`Capabilities []string`** (stable capability IDs like
     `calendar.free_busy.read`, *decoupled from version* — the join key to IAM grant/policy/audit),
     `Classification` + `SourcePolicy` (most-restrictive-wins), `AuthMode`
     (`local-native|brokered|service-side|governed-platform`), `AllowedEnvironments`/`AllowedRuntimes`,
     `TokenAudience`. **`Tags` (discovery/collection) and `Capabilities`/`Mutates` (policy) are different axes —
     don't conflate them.** Batteries stay **plain env vars**; the architecture's richer secret-bindings
     (`provider_ref`, brokered/service-side injection) is a named future gap (§9), **not** bolted onto batteries.
2. **Implement the flat install commands** (replace the `cmd/store.go` stub; **drop the `store` namespace**):
   `buttons install <name | tag:x>` / `search` / `info` / `publish` / `import`, and extend `buttons update`
   to refresh **binary + installed buttons**. Backed by `internal/store` that fetches from the registry
   `Source` (#275), **copies** each button folder into `ButtonsDir()` (+ its `requires`), and stamps
   `Source`/`Version`/`ContentHash` into `button.json`. **Reuse `update.go`'s fetch + SHA256 + atomic‑swap**
   (factor into `internal/updater` so binary + content share one path).
3. **Background auto‑update** (the "no manual step, like Claude Code"):
   ...
   - `buttons update --background` → throttled, **silent on Homebrew/permission errors** (never nag the
     ...
     **and** `buttons update --all` (content).
...
4. **Resolution fixes (correctness):** (a) **flat‑namespace collision** → namespace‑scope button names
   (`<author>/<name>`) in `paths.go`/resolver; (b) **project‑vs‑global is winner‑take‑all and the docs are
   wrong about it** (code returns only the nearest `.buttons/`, never a union with `~/.buttons/`) → make
   it union/layered (or an explicit `--global`) so a globally‑installed autono button isn't shadowed by any
   project `.buttons/`; (c) **resolution is name‑scoping only — it MUST NOT widen *data* scope:** a
   globally‑installed button still binds each invocation to a **single principal's brain/lake partition**,
...
2. **Secrets → batteries.** Rewrite `pd-proxy-request`'s `load_env` (and Plaid/Harvest/QBO peers) to read
   `BUTTONS_BAT_*` (keep the repo‑`.env` walk as a dev fallback). The button's `requires_batteries[]` then
...
3. **Kill cwd‑coupled peer resolution.** Drop the `cwd=…/autono-connectors` override in
   ...
   via `requires` (works once Track A's resolver is cwd‑independent + namespace‑scoped).
4. **Dedupe name collisions** before publishing (or rely on Track A's namespace‑scoping).
5. **Prose move:** each button's `AGENT.md` carries its prose (already there). The skill-level playbook has no
   tag-level home — fold the key workflow notes into the connectors button's `AGENT.md` (or a drawer); per‑button `AGENT.md` is
   ...
   markers, not copying `CLAUDE.md` into buttons.

---

## 5. Packaging: how the ~124 buttons become collections

The unit is the **button**; a **collection is a tag** — no "pack" object. Each former skill becomes a tag
the buttons share, so `buttons install tag:autono-cal` installs that set + each button's `requires`:

- Tag buttons by domain: `autono-cal`, `autono-finances`, `autono-meetings`, … `buttons install tag:autono`
  grabs everything; `tag:autono-cal` grabs one domain.
- The **`pd-*` connectors** are the shared dependency seam — every sync button that needs `pd-proxy-request`
  declares it in `requires`, so installing one button pulls its deps regardless of tag.
- A real **workflow** (chained buttons) is a **drawer**, distributed as a drawer — not a tag.

Buttons publish to the **registry** (R2 + signed tarballs; #275): a CI `publish` step tarballs each button +
SHA256 and updates the index. **No per-collection artifact — the collection is just the tag query.** Each
button is versioned + pinned individually; the lock file is the versioned set (rollback = restore the lock).
...
launchd  sh.buttons.autoupdate.plist  (StartInterval 1h, RunAtLoad)
   ...
   └─ buttons update --all            # button content (new, same fetch+sha256+atomic-swap)
        ↑ pulls the catalog + signed button tarballs from the registry (one bearer key; #275)
...
- **Yank reaches installed buttons** — the updater honors a `yanked` signal and rolls a machine off a revoked
...
- **Structured audit, not a free‑text log** — install/update/press/publish each emit an event into the minimal
  ...
  config_version=name@version, content_hash, decision/result-class }` (`button.installed`, `button.updated`,
  `runtime.synced`, `button.pressed`, `button.published`). **IDs + versions + result‑class only — never battery
...
|---|---|---|
| Control plane / config + tool registry (mount‑time) | `buttons install` · `button.json` + `tags` · the registry — distribute + version *which* button surfaces a runtime may mount | **config — no execution** |
...
| Secret bindings | `batteries` are **env vars** a button needs (`requires_batteries` = key names, never values). The architecture's richer binding contract (`provider_ref`, brokered/service-side) is a future gap, not modeled on batteries today. | config |
| Runtime adapter | Buttons‑on‑local is **one** adapter (generic‑local); the button format stays runtime‑neutral so OpenClaw/Codex/Hermes/Docker adapters render the same buttons into native config (e.g. bindings → OpenClaw SecretRefs). The Buttons MCP server is one **mount**, not THE adapter. | config→native |
| Observability spine | installed `name@version` + `content_hash` **are** the `config_version`/`toolset_version`; every press + OTA + publish stamps them into a structured audit event | telemetry |

**Mount‑time vs. call‑time — the split the store does NOT cross.** `buttons install` is *mount‑time*:
...
**Possession ≠ authorization.** Installing a button (or tag) and holding its batteries is credential *possession* — not
...
executes*, injecting per‑principal partition/scope). `button.json` declares only the **coarse adapter surface** —
...
**Resolved runtime contract — partial.** `store.lock` (installed + pinned buttons) is a *partial* resolved
...
### How buttons explain the canonical examples (the design test suite)
- **Ex.6 local personal coding agent** — self‑contained buttons mount **local‑provider‑native** tools, no
  platform IAM in the hot path. *The plan's sweet spot.*
- **Ex.1 Slack CoS free/busy** — installing the `autono-cal` tag only **mounts** the surface; the per‑call
...
- **Ex.18 ephemeral remote agent** — button pins + batteries give boot‑time materialization, but
  **accountability/audit must survive teardown**: install/update/press emit durable central events
  (machine, `name@version`, `content_hash`), not just a local log.
...
> trivial button, THEN migrate our buttons onto it.** Rationale: the store is the foundation (no
> versioning/install/OTA without it); a manual‑copy "proof" only re‑proves the `desk` binary and never
> exercises the real path; and we must not migrate 124 buttons against a contract (the install format,
...
|---|---|---|---|
| **1. Schema** | `buttons` | `Version`/`Tags`/`Requires`/`RequiresBatteries` on `button.json` (no `pack.json`) | the versioned/pinnable unit exists ✓ (#186) |
| **2. Install** | `buttons` | `buttons install / update` (`#274`, reuse `update.go`'s fetch+SHA256+atomic‑swap) + resolver fixes (namespace‑scoped names, global∪project union) | one‑command install works |
| **3. Validate** | both | a **trivial throwaway button** installs + updates + auto‑updates end‑to‑end | go/no‑go: "buttons + store actually solve it" *before* mass migration |
| **4. Migrate** | `desk` | our buttons → tagged + published: `desk` ✓ · secrets→batteries · drop cwd hacks · dedupe names · publish | the autono buttons ship via the registry, done **once** against the real contract |
...
2. **Define the store spec** — the code (`#274`, Phase 5.1) and `SECURITY.md` (Phase 3) disagree and
   nothing is specified. You get to define it; recommend the registry source + button tarballs + SHA256 now,
   signatures when you add untrusted installs.
3. **v1 scope** — start with `autono-connectors` (base) + the brain‑query/`pd-*` self‑contained
   buttons; defer the TS‑CLI sync buttons to Phase 0's completion.
4. **Namespace‑scoping vs author‑time prefixes** for the name collisions — recommend the engine namespace‑scope
...
8. **Prompt/model/persona axis** — the resolved contract should pin these too; today they live in
   `CLAUDE.md`/`agentskill`. Should they become a button-set (or a sibling agent‑definition artifact)?
...
10. **Hot‑swap + session/catalog invalidation** — `store update` must not mutate a button mid‑press (in‑flight
    ...
    running one.
11. **Yank / revocation reaches installed buttons** — a `yanked` signal the background updater honors
    (fail‑closed), not just blocking new installs.
12. **Non‑local runtime adapters** — OpenClaw/Codex/Hermes/Docker render the same buttons into native config
    (tool policy, SecretRefs, sandbox profiles); out of v1, but the button/tag format must not bake in
