...
var Targets = []Target{
	...
	},
	{
		ID:          "openclaw",
		Label:       "OpenClaw",
		Description: "Appends a section to AGENTS.md (OpenClaw workspace operating instructions)",
		Path:        "AGENTS.md",
		Format:      formatMarkdownSection,
	},
	{
		ID:          "hermes",
		Label:       "Hermes",
		Description: "Appends a section to AGENTS.md (Hermes reads the AGENTS.md convention)",
		Path:        "AGENTS.md",
		Format:      formatMarkdownSection,
	},
}
...
// List markers delimit the auto-generated, per-project button list. It is a
// SEPARATE managed block from the BUTTONS section above: `buttons init` rewrites
// the BUTTONS section, while `buttons create` rewrites this list, so the two
// never clobber each other.
const (
	listMarkerStart = "<!-- BUTTONS:LIST:START -->"
	listMarkerEnd   = "<!-- BUTTONS:LIST:END -->"
)
...
// ButtonEntry is the minimal button info rendered into the AGENTS.md project
// button list. Kept local so this package stays dependency-free.
type ButtonEntry struct {
	Name        string
	Description string
}

// ButtonListCap is how many buttons render with a full description line. The
// caller passes buttons most-pressed-first, so the cap keeps the hottest
// buttons detailed; the rest collapse to a comma-separated name list. Keeps
// AGENTS.md scannable in workspaces with hundreds of buttons.
const ButtonListCap = 20

// RenderButtonList renders the body between the LIST markers. The first
// ButtonListCap buttons render in `buttons summary` shape — the press command
// and its description; the caller orders them most-pressed-first. Any beyond
// the cap collapse to one comma-separated line of names pointing at
// `buttons list` for the full descriptions.
func RenderButtonList(buttons []ButtonEntry) string {
	var b strings.Builder
	b.WriteString("## Buttons in this project\n\n")
	b.WriteString("<!-- Auto-generated by the buttons CLI; edits between the markers are overwritten. -->\n\n")
	if len(buttons) == 0 {
		b.WriteString("_No buttons yet — `buttons create <name>`._")
		return b.String()
	}
	shown, overflow := buttons, []ButtonEntry(nil)
	if len(buttons) > ButtonListCap {
		shown, overflow = buttons[:ButtonListCap], buttons[ButtonListCap:]
	}
	for _, btn := range shown {
		b.WriteString("- `buttons press " + btn.Name + "`")
		if d := strings.TrimSpace(btn.Description); d != "" {
			b.WriteString(" — " + d)
		}
		b.WriteString("\n")
	}
	if len(overflow) > 0 {
		...
		fmt.Fprintf(&b, "\n_Plus %d more: %s — run `buttons list` for full descriptions._\n", len(overflow), strings.Join(names, ", "))
	}
	return strings.TrimSuffix(b.String(), "\n")
}

// WriteButtonList writes (or updates in place) the auto-generated project button
// list in <projectRoot>/AGENTS.md, wrapped in BUTTONS:LIST markers. Idempotent,
// mirroring writeMarkdownSection: replace between LIST markers if present, else
// insert right after the BUTTONS section, else append, else create the file.
func WriteButtonList(projectRoot string, buttons []ButtonEntry) (WriteResult, error) {
	path := filepath.Join(projectRoot, "AGENTS.md")
	section := listMarkerStart + "\n" + RenderButtonList(buttons) + "\n" + listMarkerEnd + "\n"

	existing, err := os.ReadFile(path) // #nosec G304 -- projectRoot is the resolved project dir; filename is constant
	if err != nil {
		if os.IsNotExist(err) {
			if err := os.WriteFile(path, []byte(section), 0o600); err != nil { // #nosec G306 -- 0600 is intended
				return WriteResult{}, fmt.Errorf("write %s: %w", path, err)
			}
			return WriteResult{TargetID: "agents-md", Path: path, Action: "created"}, nil
		}
		return WriteResult{}, fmt.Errorf("read %s: %w", path, err)
	}
	content := string(existing)

	if strings.Contains(content, listMarkerStart) && strings.Contains(content, listMarkerEnd) {
		s := strings.Index(content, listMarkerStart)
		e := strings.Index(content, listMarkerEnd) + len(listMarkerEnd)
		if e > s {
			updated := content[:s] + strings.TrimSuffix(section, "\n") + content[e:]
			if err := os.WriteFile(path, []byte(updated), 0o600); err != nil { // #nosec G306 G703 -- 0600 intended; AGENTS.md is a constant filename under the resolved projectRoot (no traversal)
				return WriteResult{}, fmt.Errorf("write %s: %w", path, err)
			}
			return WriteResult{TargetID: "agents-md", Path: path, Action: "updated"}, nil
		}
	}

	if idx := strings.Index(content, markerEnd); idx >= 0 {
		at := idx + len(markerEnd)
	...
	b.WriteString(section)
	if err := os.WriteFile(path, []byte(b.String()), 0o600); err != nil { // #nosec G306 G703 -- 0600 intended; AGENTS.md is a constant filename under the resolved projectRoot (no traversal)
		return WriteResult{}, fmt.Errorf("write %s: %w", path, err)
	}
	return WriteResult{TargetID: "agents-md", Path: path, Action: "appended"}, nil
}
